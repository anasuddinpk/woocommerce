...
describe( 'useCategorySearch', () => {
	...
	it( 'should sort items by count first and then alphabetical', async () => {
		...
		expect( result.current.categoriesSelectList[ 0 ].label ).toEqual(
			'ZZZ'
		);
		expect( result.current.categoriesSelectList[ 1 ].label ).toEqual(
			'AA'
		);
		expect( result.current.categoriesSelectList[ 2 ].label ).toEqual(
			'BB'
		);
	} );
	...
	it( 'should order the select list by parent, child, nested child, parent', async () => {
		...
		expect( result.current.categoriesSelectList[ 3 ].label ).toEqual(
			'Accessories'
		);
		// child of accessories.
		expect( result.current.categoriesSelectList[ 4 ].label ).toEqual(
			'Belts'
		);
		// child of clothing.
		expect( result.current.categoriesSelectList[ 5 ].label ).toEqual(
			'Hoodies'
		);
		// top level.
		expect( result.current.categoriesSelectList[ 6 ].label ).toEqual(
			'Furniture'
		);
	} );
